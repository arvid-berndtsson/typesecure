...
const TYPESECURE_SYMBOL: unique symbol = Symbol.for('typesecure.classified') as unknown as typeof TYPESECURE_SYMBOL;
...
function makeClassified<K extends DataClassification, T>(kind: K, value: T): Classified<K, T> {
  return { kind, value, [TYPESECURE_SYMBOL]: true } as const;
}

export function isClassified(value: unknown): value is Classified<DataClassification, unknown> {
  return (
    typeof value === 'object' &&
    value !== null &&
...
export const PublicStringSchema = NonEmptyString.transform((v) => makeClassified('public', v));
export const PIIStringSchema = NonEmptyString.transform((v) => makeClassified('pii', v));
export const SecretStringSchema = NonEmptyString.transform((v) => makeClassified('secret', v));
export const TokenStringSchema = NonEmptyString.transform((v) => makeClassified('token', v));
export const CredentialStringSchema = NonEmptyString.transform((v) => makeClassified('credential', v));
