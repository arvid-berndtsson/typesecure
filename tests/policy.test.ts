...
describe("Policy", () => {
  test("default policy denies logging secrets and pii", () => {
    ...
    expect(() =>
      assertAllowed(policy, "log", { s: secretText("x") }),
    ).toThrow();
    expect(() => assertAllowed(policy, "log", { p: piiText("y") })).toThrow();
    expect(() =>
      assertAllowed(policy, "log", { ok: publicText("hello") }),
    ).not.toThrow();
  });
  ...
  test("default policy allows tokens over network", () => {
    const policy = defaultPolicy();
    expect(() =>
      assertAllowed(policy, "network", { token: token("abc") }),
    ).not.toThrow();
  });
  ...
  test("policyLog redacts (instead of leaking) when allowed", () => {
    ...
    expect(() =>
      policyLog(policy, logger, "log", publicText("hello"), {
        token: token("abc"),
      }),
    ).toThrow();
